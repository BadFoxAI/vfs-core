...
impl MiniCC {
    ...
    fn compile_func(&mut self) {
        ...
        if self.consume() == Token::Char { is_byte = true; } 
...
pub fn run_suite() -> String {
    ...
    // TEST: FULL SOVEREIGN COMPILER PIPELINE
    // This C-Subset program takes a simulated token array (IDENT, ASSIGN, NUM, SEMI), 
    // applies grammar rules to parse it, and directly generates executable 
    // DRE Bytecode (PUSH, LSTORE) into a simulated executable memory segment.
    report.push_str("TEST: SOVEREIGN_COMPILER_PIPELINE ... ");
    let src = "
    int main() {
        // 1. Setup Token Stream (Simulating output of Tokenizer)
        // [2: IDENT, 4: ASSIGN, 1: NUM, 5: val, 5: SEMI, 0: EOF]
        int *t = 12288;
        *t = 2; t = t + 8; 
        *t = 4; t = t + 8; 
        *t = 1; t = t + 8; 
        ...
        *t = 0;            
            ...
            if (*read_t == 2) {
                ...
                if (*next == 4) {
                    ...
                    if (*num_tok == 1) {
                        ...
                        // Emit Bytecode for 'x = 5;' -> PUSH 5, LSTORE 0
                        *code = 16; code = code + 1; len = len + 1; // 0x10 PUSH
                        *code = val; code = code + 1; len = len + 1; // 5
                        *code = 97; code = code + 1; len = len + 1; // 0x61 LSTORE
                        *code = 0; code = code + 1; len = len + 1; // offset 0
        ...
        // 3. Verify Generated Bytecode Integrity
        char *check = 13000;
        int sum = 0;
        int i = 0;
        while (i < len) {
        ...
        // Expected Sum: 16 (PUSH) + 5 + 97 (LSTORE) + 0 = 118
        return sum;
    }
    ";
